...
export function generateOAuthState(): string {
  ...
  if (typeof window !== 'undefined' && window.localStorage) {
    window.localStorage.setItem('naver_oauth_state', state);
  }
  return state;
}
...
export function verifyOAuthState(receivedState: string | null): boolean {
  ...
  const storedState = window.localStorage.getItem('naver_oauth_state');
  window.localStorage.removeItem('naver_oauth_state'); // 1회용 소비
